...
const ruleTesterV8 = new RuleTesterV8({
  env: { es2020: true },
  ...
  for (const flavour in config.configs) {
    describe(`${flavour} config`, () => {
      it("exports valid rules", () => {
        ...
        const ceRules = Object.keys(config.configs[flavour].rules || []).filter(
          (rule) => rule.startsWith("escompat/"),
        );
        const violations = ceRules.filter(
          (rule) => !exportedRules.has(rule.replace(/^escompat\//, "")),
        );
        assert.deepStrictEqual(
          violations,
          [],
          "All custom-elements/ rules should exist in lib/index.js#rules",
        );
      });
    });
  }
...
describe("test coverage", () => {
  it("has tests for each rule and rules for each test", () => {
    ...
    assert.deepStrictEqual(
      ...
      tests,
      "Expected lib/rules/*.js to have same files as test/*.js",
    );
  });
});
...
describe("documentation", () => {
  ...
  it("has readme link to each doc", () => {
    ...
    let n = contents.findIndex(
      (line, index) => index > i && line.startsWith("#"),
    );
    ...
    const ruleLinks = contents
      ...
      .filter(Boolean)
      .map((x) => x.trim());
    const desiredRuleLinks = rulesFromDir(docDir).map(
      (rule) => `- [${rule}](${docDir}/${rule}.md)`,
    );
    assert.deepStrictEqual(
      desiredRuleLinks,
      ruleLinks,
      `Expected each rule in ${docDir}/*.md to have README link`,
    );
  });
  ...
  for (const doc of rulesFromDir(docDir)) {
    it(`has correct headings in ${doc}.md`, () => {
      const contents = fs
        .readFileSync(`${docDir}/${doc}.md`, "utf-8")
        .split("\n");
      let consume = true;
      ...
      const desiredHeadings = [`# ${doc}`, "## What is the Fix?"].filter(
        Boolean,
      );
      assert.deepStrictEqual(
        headings,
        desiredHeadings,
        "Expected doc to have correct headings",
      );
    });
    ...
    it(`has working examples in ${doc}.md`, () => {
      ...
      for (const { code, startLine } of extractCodeblocks(lines)) {
        const validIndex = lines.lastIndexOf(
          "👍 Examples of **correct** code for this rule:",
          startLine,
        );
        const invalidIndex = lines.lastIndexOf(
      ...
      ruleTesterV8.run(doc, rule, rules);
    });
    ...
    it(`has javascript examples in ${doc}.md`, () => {
      ...
      for (const { lang, startLine } of extractCodeblocks(lines)) {
        assert.equal(
          lang,
          "js",
          `Expected codeblock on line ${startLine} to equal "js"`,
        );
      }
    });
  }
